<template>
  <ion-page>
    ...
    <ion-content class="flex flex-col justify-between pb-safe">
      <ion-list class="list-large-items">
        ...
        <ion-item button @click="changePassword">
          <ion-icon slot="start" :src="require('ionicons5/dist/svg/lock-closed-outline.svg')"
                    color="dark"></ion-icon>
    ...
    <ion-footer>
      <ion-item button class="large-item" detail="false" lines="none" @click="deleteAccount">
  ...
  deleteAccount() {
    ModalPresenter.present(this.$ionic, DeleteAccountModal, {
      t: {
        ...
        deactivateMessage: this.$t('deactivate-account-message'),
        deactivateConfirmation: this.$t('deactivate-account-confirmation'),
        success: this.$t('deactivate-account-success'),
